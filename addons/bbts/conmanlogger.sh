...
{
  echo -e "\t init $1"
  $SCRIPT_BIN -f -c "$CONMAN_BIN -d $CONMAN_SERVER $1" \
...
function kill_monitor()
{
  ...
  ppid=$(ps -u $USER -o pid= -o command= | grep "^[0-9]\+ $SCRIPT_BIN" | grep $(get_logfile $2 $1) | awk '{print $1}')
  pids=()
...
#function list_monitor()
#{
#  ps -u $USER -o pid= -o command= | grep "^[0-9]\+ $SCRIPT_BIN"
#}
