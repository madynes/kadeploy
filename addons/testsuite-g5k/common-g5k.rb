...
def ssh(user,host,cmd,password='grid5000')
...
def kabootstrap(nodefile,daemon,commit,version)
  bin=KABOOTSTRAP_BIN
  frontend=nil
  ret=nil
  i = 0
  begin
    ret=cmd("#{bin} #{KABOOTSTRAP_KERNELS} #{KABOOTSTRAP_ENVS} -f #{nodefile} -d #{daemon} --git #{commit} -v #{version} #{ENV['KABOOTSTRAP_OPTS']} ",false)
