...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \yfieldd{size}{Integer}{8000} size (MB) of the Kadeploy cache.
  \end{itemize}

  \item \ypath{/network}
  \begin{itemize}
    \item \yfieldd{server\_hostname}{String}{result of hostname command} hostname of the Kadeploy server
...
The information used to generate this filenames are the one specified for each nodes in the clusters configuration file (see section \ref{sec:clusters_conf}). For example, with PXElinux, it will be \emph{ip\_hex}.
  ...
  \begin{itemize}
    \item \yfieldd{connector}{String}{SSH_COMMAND} connector used by Taktuk
