...
\vspace*{18cm}
\noindent Copyright \copyright by INRIA, Emmanuel Jeanvoine, emmanuel.jeanvoine@inria.fr 2008-2011\\
...
rights_kind = db
taktuk_connector = ssh -A -l root -q \
...
\item \texttt{taktuk\_connector}: connector used by Taktuk.
...
\item \texttt{async\_end\_of\_reboot\_hook}: command to launch at the end of an asynchronous reboot. The REBOOT\_ID can be used in the command, it is replaced at the runtime.
\item \texttt{async\_end\_of\_power\_hook}: command to launch at the end of an asynchronous power operation. The POWER\_ID can be used in the command, it is replaced at the runtime.
\item \texttt{vlan\_hostname\_suffix}: this specifies the suffix to add to the hostname to define the hostname in the given VLAN. The pattern VLAN\_ID can be used in the definition, it is replaced at the runtime.
\item \texttt{set\_vlan\_cmd}: command to launch in orded to put a set of nodes in a VLAN. The patterns NODES, USER and VLAN\_ID can be used.
\end{itemize}
...
\item \texttt{timeout\_reboot\_classical}: classical reboot timeout. A Ruby expression can be used here to represent a function depending on n (the number of nodes currently rebooted).
\item \texttt{timeout\_reboot\_kexec}: kexec reboot timeout. A Ruby expression can be used here to represent a function depending on n (the number of nodes currently rebooted).
\item \texttt{cmd\_soft\_reboot}: generic soft reboot command. You can use the HOSTNAME\_FQDN and HOSTNAME\_SHORT variables in the command-line.
...
\item \texttt{cmd\_soft\_power\_on}: generic soft power on command. You can use the HOSTNAME\_FQDN and HOSTNAME\_SHORT variables in the command-line.
\item \texttt{cmd\_hard\_power\_on}: generic hard power on command. You can use the HOSTNAME\_FQDN and HOSTNAME\_SHORT variables in the command-line.
\item \texttt{cmd\_very\_hard\_power\_on}: generic very hard power on command. You can use the HOSTNAME\_FQDN and HOSTNAME\_SHORT variables in the command-line.
\item \texttt{cmd\_soft\_power\_off}: generic soft power off command. You can use the HOSTNAME\_FQDN and HOSTNAME\_SHORT variables in the command-line.
\item \texttt{cmd\_hard\_power\_off}: generic hard power off command. You can use the HOSTNAME\_FQDN and HOSTNAME\_SHORT variables in the command-line.
\item \texttt{cmd\_very\_hard\_power\_off}: generic very hard power off command. You can use the HOSTNAME\_FQDN and HOSTNAME\_SHORT variables in the command-line.
\item \texttt{cmd\_power\_status}: generic power status command. You can use the HOSTNAME\_FQDN and HOSTNAME\_SHORT variables in the command-line.
...
\item \texttt{nfsroot\_params}: kernel parameters for the NFS-root deployment environment (only used if you use an NFS-root deployment environment)
...
\paragraph{Note about the reboot/power-on/power-off commands}
...
In the part~\ref{sec:specific_config} we saw that generic commands can be given to all the nodes that belong to a cluster. It is also possible to override these generic values for some specific nodes. To do this, you must fill the file named \texttt{cmd} in the configuration directory. Each line of the file must specify a command to override for a given node and the syntax is: \texttt{hostname}|command kind|\texttt{cmd} where \texttt{hostname} must be also defined in the \texttt{nodes} file and the expected values for command kind are:
\begin{itemize}
\item \texttt{reboot\_soft}
\item \texttt{reboot\_hard}
\item \texttt{reboot\_very\_hard}
\item \texttt{power\_on\_soft}
\item \texttt{power\_on\_hard}
\item \texttt{power\_on\_very\_hard}
\item \texttt{power\_off\_soft}
\item \texttt{power\_off\_hard}
\item \texttt{power\_off\_very\_hard}
\item \texttt{power\_status}
\item \texttt{console}
\end{itemize}
...
\end{verbatim}
For very specific purpose, you can add some custom operations in the deployment workflow. To do this, you have to specify these operations in a file where each line specify the operations that must be executed at the beginning of a micro-step. The syntax is: \texttt{macrostep,microstep@cmd1\%arg\%dir,cmd2\%arg\%dir,...,cmdN\%arg\%dir}. You can specify two kinds of operations: \textit{send} and \textit{exec}. For the \textit{send} operation, \texttt{arg} is used to specify a file name and \texttt{dir} is used to specify the destination directory. Concerning the \textit{exec} operation, \texttt{arg} is used to specify the command to execute and \texttt{dir} must be empty.
...
You can do want you want in the pre-install but you must know that Kadeploy will extract the environment in the directory defined by the \texttt{environment\_extraction\_dir} field of the general configuration file. Commonly, this directory is \texttt{/mnt/dest}. Thus, you have to mount all the partitions you need in this directory. If you wish to deploy the environment onto several partitions, you can use for instance the following map:
\begin{itemize}
