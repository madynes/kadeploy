...
\begin{small}
\begin{verbatim}
> dpkg -i kadeploy-v3-common-3-0.0.deb
> dpkg -i kadeploy-v3-client-3-0.0.deb
\end{verbatim}
\end{small}
...
The following installation method works only an Fedora based distribution. We assume that you have a configured rpm build environment. Furtermore, Taktuk must be installed on the server side.
\subsubsection{Build}
\noindent First, you have to uncompress the Kadeploy tarball. 
\begin{small}
\begin{verbatim}
...
\begin{small}
\begin{verbatim}
> /usr/local/kadeploy3/sbin/kadeploy3d
\end{verbatim}
\end{small}
...
> kareboot3 -f nodes -r back_to_prod_env
...
\end{verbatim}
This command allows to define the environemnt \textit{sidx64-base} version 3 as a private environment. Note that the environment version is required and only the almighty environment users are allowed to define an environment as public.
