...
\begin{itemize}
\item \texttt{ruby1.8}
\item \texttt{rubylibmysql-ruby1.8}
...
\begin{small}
\begin{verbatim}
> tar xzf kadeploy-3.0-1.tar.gz -C DESTINATION_DIR
\end{verbatim}
\end{small}
...
\end{verbatim}
\end{small}
This will generate three Debian package: \texttt{kadeploy-common-3.0-1.deb}, \texttt{kadeploy-client-3.0-1.deb}, and \texttt{kadeploy-server-3.0-1.deb}.
\subsubsection{Installation}
\noindent On the server side, you have to install the \texttt{kadeploy-common-3.0-1.deb} and \texttt{kadeploy-server-3.0-1.deb} packages.
\begin{small}
\begin{verbatim}
> dpkg -i kadeploy-common-3.0-1.deb
> dpkg -i kadeploy-server-3.0-1.deb
\end{verbatim}
\end{small}

\noindent On the client side, you have to install the \texttt{kadeploy-common-3.0-1.deb} and \texttt{kadeploy-client-3.0-1.deb} packages.
\begin{small}
\begin{verbatim}
> dpkg -i kadeploy-common-3.0-1.deb
> dpkg -i kadeploy-client-3.0-1.deb
\end{verbatim}
\end{small}
...
\end{verbatim}
\end{small}
This will generate three rpm package in the RPMS package of your build environment, for instance: \texttt{kadeploy-client-3.0-1.noarch.rpm}, \texttt{kadeploy-server-3.0-1.noarch.rpm}, and \texttt{kadeploy-common-3.0-1.noarch.rpm}.
\subsubsection{Installation}
\noindent On the server side, you have to install the \texttt{kadeploy-common-3.0-1.noarch.rpm} and \texttt{kadeploy-server-3.0-1.noarch.rpm} packages.
\begin{small}
\begin{verbatim}
> rpm -i kadeploy-common-3.0-1.noarch.rpm
> rpm -i kadeploy-server-3.0-1.noarch.rpm
\end{verbatim}
\end{small}

\noindent On the client side, you have to install the \texttt{kadeploy-common-3.0-1.noarch.rpm} and \texttt{kadeploy-client-3.0-1.noarch.rpm} packages.
\begin{small}
\begin{verbatim}
> rpm -i kadeploy-common-3.0-1.noarch.rpm
> rpm -i kadeploy-client-3.0-1.noarch.rpm
\end{verbatim}
\end{small}
...
\subsection{Automatic launch on a Debian based distribution}
\noindent On a Debian based distribution, the rc script can be automatically launched at the startup by launching, with root rights, the following command:
\begin{small}
\begin{verbatim}
> update-rc.d kadeploy3d defaults
\end{verbatim}
\end{small}

\subsection{Automatic launch on a Fedora based distribution}
\noindent On a Fedora based distribution, the rc script can be automatically launched at the startup by launching, with root rights, the following command:
\begin{small}
\begin{verbatim}
...
General options:
  ...
  -u, --user USERNAME            Specify the user
  -v, --version NUMBER           Specify the environment version
  -w, --set-pxe-profile FILE     Set the PXE profile (use with caution)
...
. In the Kaenv part you can find the way to reset the \textit{demolishing} counter of an environment by using an option of Kaenv.
...
Contact: kadeploy3-users@lists.grid5000.fr
General options:
  -e, --environment ENVNAME            Environment name
  -f, --file FILE                      Environment file
  -m, --files-to-move FILES            Files to move (src1:dst1,src2:dst2,...)
  -o, --operation OPERATION            Kind of operation (add, delete, list, print,
                                       remove-demolishing-tag, set-visibility-tag, 
                                       update-tarball-md5, update-preinstall-md5, 
                                       update-postinstalls-md5, move-files)
...
\begin{verbatim}
> kaenv3 -o print -e FreeBSD -v 3 -u johnsmith
\end{verbatim}
...
\begin{verbatim}
> kaenv3 -o remove-demolishing-tag -e FreeBSD -v 3
\end{verbatim}
...
\begin{verbatim}
> kaenv3 -o update-tarball-md5 -e sidx64-base
\end{verbatim}
...
\begin{verbatim}
> kaenv3 -o update-postinstalls-md5 -e sidx64-base
\end{verbatim}
...
\begin{verbatim}
> kaenv3 -o set-visibility-tag -t private -e sidx64-base -v 3
\end{verbatim}
...
\paragraph{Use case 1}
\begin{verbatim}
> kanodes3 -o get_deploy_state
\end{verbatim}
This commands prints the global state of all the nodes managed by a Kadeploy server.

\paragraph{Use case 2}
\begin{verbatim}
> kanodes3 -o get_deploy_state -m gdx-25.orsay.grid5000.fr -f machine_file
\end{verbatim}
