...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \yfield{server}{String} The server where PXE files are stored.
  \end{itemize}
  \item \ypath{/pxe/dhcp/profiles}
  \begin{itemize}
    \item \yfield{directory}{String} The directory where PXE profiles have to be written. This path is relative to the PXE repository path unless you specify an absolute path.

For sample, with PXElinux, this directory is \emph{pxelinux.cfg}.
...
The information used to generate this filenames are the one specified for each nodes in the clusters configuration file (see section \ref{sec:clusters_conf}). For sample, with PXElinux, it will be \emph{ip\_hex}.
  \end{itemize}
  \item \ypath{/pxe/dhcp/userfiles}
  \begin{itemize}
  ...
  \item \ypath{/pxe/networkboot} the method used to PXE boot OS images send by the network. This setting is optional by default, the DHCP method is used.
  \begin{itemize}
    \item \yfield{method}{String} the PXE method used to PXE boot (expected values are PXElinux, GPXElinux, IPXE or GrubPXE)
    \item \yfield{binary}{String} the binary used by this method (if this method is different than the DHCP one, this file will be loaded by the PXE method). For sample, for PXElinux, this file is \emph{pxeliux.0}.
    \item \yfield{repository}{String} absolute path of the repository where PXE files are accessibles (TFTP, HTTP, ...). Warning, as far as the Kadeploy server is launched by the \texttt{deploy} user, \texttt{deploy} must have the rights to write in this directory.
  \end{itemize}
...
The information used to generate this filenames are the one specified for each nodes in the clusters configuration file (see section \ref{sec:clusters_conf}). For sample, with PXElinux, it will be \emph{ip\_hex}.
  ...
  \item \ypath{/pxe/localboot} the method used to PXE boot system that are installed on nodes hard disk. This setting is optional by default, the DHCP method is used.
  \begin{itemize}
    \item \yfield{method}{String} the PXE method used to PXE boot (expected values are PXElinux, GPXElinux, IPXE or GrubPXE)
    \item \yfield{binary}{String} the binary used by this method (if this method is different than the DHCP one, this file will be loaded by the PXE method). For sample, for PXElinux, this file is \emph{pxeliux.0}.
    \item \yfield{repository}{String} absolute path of the repository where PXE files are accessibles (TFTP, HTTP, ...). Warning, as far as the Kadeploy server is launched by the \texttt{deploy} user, \texttt{deploy} must have the rights to write in this directory.
...
As far as you are using Kadeploy3 on your cluster, a PXE boot have to be setup on your nodes, so that implies that your nodes have compatible network interface cards and that your DHCP server is configured a specific way. There is a lot of different softwares that you can use to make your nodes boot over the network: PXElinux, GPXElinux, iPXE, Etherboot, Grub disks, ... .

For almost all this softwares, the booting method is the same:
\begin{enumerate}
  \item The network iterface card ask an IP address to the DHCP server;
  \item The DHCP answer and give an extra instruction that specifies to download and run a specific file, the software (for sample, with PXElinux, this file is pxelinux.0)
  \item The software download a profile on the network that specifies how to boot the node. The profile file has a specific name (the hostname of the node, it's IP address, ...) so that the software will be sure to download the profile of this specific node.
  \item The software read the profile and execute it's instructions (download and boot a specific kernel, boot on node's local hard disk, ...)
\end{enumerate}

To be able to control node's boot, Kadeploy3 is editing this PXE profiles. That's why you have to tell it what kind of software you are using to boot your nodes over the network and how you want their profiles to be written.
...
In the field \ypath{/pxe/dhcp} of the global configuration file, you will specify which software is setup on your DHCP server and give some information about the exports and profiles. This settings will be used every times Kadeploy3 will reboot your nodes unless you defile the fields \ypath{/pxe/networkboot} or \ypath{/pxe/localboot}.
...
When the method specified in \ypath{/pxe/networkboot} or \ypath{/pxe/localboot} is different than the method specified \ypath{/pxe/dhcp}, Kadeploy3 make the PXE boot software chainload each other. For sample if \ypath{/pxe/dhcp} is set to boot with PXElinux and \ypath{/pxe/networkboot} with a GRUB disk \emph{grubpxe.0}, Kadeploy3 will make PXElinux load \emph{grubpxe.0} by writting \emph{PXE grubpxe.0} in it's profile, and then write the GRUB profiles that will make the node download and boot a kernel from the network.

One important thing is that every PXE booting softwares executables have to be available in you PXE (TFTP in most of the cases) repository. Another one is that the Kadeploy user should be able (have the rights) to write the profiles inside the PXE repository.
