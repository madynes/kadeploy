...
\begin{itemize}
  ...
  \item \ypath{/authentication/certificate} authentication the certificate of a Certification Authority, at least a public key or a x509 certificate as to be specified.
  \begin{itemize}
  ...
  \item \ypath{/authentication/secret\_key} authentication using a secret key.
  \begin{itemize}
    \item \yfield{key}{String} The secret key that has to be specified by the user to be identified.
  \end{itemize}
...
Advanced options:
      ...
      --secure                     Use a secure connection to export files to the server
...
General options:
  -b, --block-device BLOCKDEVICE Specify the block device to use
  -c, --check-destructive-tag    Check if some nodes was deployed with an environment that have the destructive tag
...
General options:
  ...
  -u, --user USERNAME                   Specify the user
      ...
      --server STRING                   Specify the Kadeploy server to use
Advanced options:
      --toggle-destructive-tag ENVNAME  Toggle the destructive tag on an environment
      --set-visibility-tag ENVNAME      Set the visibility tag on an environment
      --update-image-checksum ENVNAME   Update the checksum of the environment image
      --update-preinstall-checksum ENVNAME   Update the checksum of the environment preinstall
      --update-postinstalls-checksum ENVNAME Update the checksum of the environment postinstalls
...
Contact: kadeploy3-users@lists.grid5000.fr
General options:
  -d, --get-nodes-state        Get the deploy state of the nodes
  -s, --get-deployments-status Get the status of running deployments
  -f, --file MACHINELIST       Only print information about the given machines (- means stdin)
  -m, --machine MACHINE        Only print information about the given machines
  -v, --version                Get the version
  -w, --workflow-id WID        Specify a workflow id (to be used with the get_status option). If no wid is specified, the information of all the running deployments will be returned.
      --server STRING          Specify the Kadeploy server to use
\end{verbatim}
\end{small}
...
\paragraph{File fetching errors}
\begin{itemize}
...
\begin{itemize}
  ...
  \item \texttt{302}: Cannot reboot since the nodes have been previously deployed with a destructive environment
...
\end{itemize}
\subsection{Exporting files for the server\label{sec:api_files_export}}
'client' field specification, ...
\subsection{Request's parameters}
Via query string or request's body, ...
\subsection{On the server-side}

\section{Specifications}
\subsection{Authentication}
admin/user: ident,secret\_key,ca\_cert, ...
\subsection{Global}
\begin{itemize}
  \item GET /version
  \item GET /info
...
\begin{itemize}
  \item POST /deploy (wid/resources)
  \item GET /deploy/\#dpl\#
  \item GET /deploy/\#dpl\#/state
  \item GET /deploy/\#dpl\#/outputs
  \item GET /deploy/\#dpl\#/outputs/\#cluster\#
  \item GET /deploy/\#dpl\#/debugs
  \item GET /deploy/\#dpl\#/debugs/\#node\#
\end{itemize}
\subsection{Reboot operations}
