...
\begin{itemize}
  ...
  \item \yfield{destination}{String} \small{(To be specified if the action is \emph{send})} The destination directory on the nodes (the file name will remains the same)
...
If you specify \texttt{parted} for the \ypath{/partitioning/kind} field in the cluster-specific configuration file, you must provide a file containing a parted map. Currently, only the Linux partition kind is handled with parted.
