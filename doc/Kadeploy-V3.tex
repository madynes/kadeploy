...
\begin{itemize}
  ...
  \item \yfield{file}{String} \small{(To be specified if the action is \emph{send})} The path to the file to be send (Kadeploy3 specific environment variables are substitued in the path)
...
If you specify \texttt{fdisk} for the \ypath{/partitioning/kind} field in the cluster-specific configuration file, the partition map file must be a fdisk map. In this case, concerning the kind of the deployment partition, you must use the value \texttt{PARTTYPE} instead of a real value. Thus, this value will be modified on the fly according to the kind of environment deployed (field \texttt{fdisktype} of the environment description).

If you specify \texttt{parted} for the \ypath{/partitioning/kind} field in the cluster-specific configuration file, you must provide a file containing a parted map. In this case, concerning the kind of the deployment partition, you must use the value \texttt{PARTTYPE} instead of a real value. Thus, this value will be modified on the fly according to the kind of environment deployed (field \texttt{filesystem} of the environment description).
...
When you are executing a command or a script (via the \texttt{exec} action), you can use the following environment variables :
\begin{itemize}
...
\texttt{Note}: This strings will also be substitued in your directory specification when you are using the \texttt{send} action.
    ...
    BroadcastEnvKastafior:
      send_environment:
        post-ops:
          - action: send
            file: my_custom_file
            destination: KADEPLOY_ENV_EXTRACTION_DIR
            retries: 1
            timeout: 10
            scattering: chain
\end{verbatim}
