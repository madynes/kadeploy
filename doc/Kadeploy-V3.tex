...
This method will be used to boot deployment environment (SetDeploymentEnv macro-step), systems installed on hard disk (BootNewEnv macro-step), or custom PXE files (option \emph{--upload-pxe-files}) if these method are not overwritten.

This setting have to fit with the file you're nodes are loading after the DHCPDISCOVER answer from your DHCP server (so it depends on your DHCP server configuration).
