...
\begin{small}
\begin{verbatim}
cmd_very_hard_power_off = /usr/sbin/very_hard_power_off GROUP_SHORT,path_to_group_of_node_for_hard_power_off_cmd
\end{verbatim}
\end{small}
...
\end{itemize}
For a given command on a given cluster, if you specify some group of nodes (with \texttt{GROUP\_SHORT} or \texttt{GROUP\_FQDN} patterns), you will also be able to specify, for some nodes of the cluster, a command that does not imply a group of nodes. To do this, you must specify these commands in the cpecific commands configuration files.
...
\begin{itemize}
\item \texttt{soft\_reboot}
\item \texttt{hard\_reboot}
\item \texttt{very\_hard\_reboot}
\item \texttt{soft\_power\_on}
\item \texttt{hard\_power\_on}
\item \texttt{very\_hard\_power\_on}
\item \texttt{soft\_power\_off}
\item \texttt{hard\_power\_off}
\item \texttt{very\_hard\_power\_off}
...
\begin{small}
\begin{verbatim}
vm-001|soft_reboot|ssh -q root@vm-001 /sbin/special_reboot_for_vm
vm-001|hard_reboot_hard|vmware-cmd /home/vmware/vm-001/vm-001.vmx reset hard
vm-002|soft_reboot|ssh -q root@vm-002 /sbin/special_reboot_for_vm
\end{verbatim}
\end{small}
