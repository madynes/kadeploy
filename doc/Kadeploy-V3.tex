...
\begin{itemize}
  ...
  \item \ypath{/kernels/user} Default options for user kernels
  \begin{itemize}
  ...
  \item \ypath{/kernels/deploy} The deployment environment
  \begin{itemize}
    ...
    \item \yfieldd{supported\_fs}{String}{''} list of filesystems that are supported by the deployment environment. The syntax is: fstype1,fstype2,fstype3,... . When deploying an environment with a non supported filesystem type, the deployment workflow will be modified (see section \ref{sec:macro_desc}).
...
\item \texttt{SetDeploymentEnv}
  ...
  \item \texttt{SetDeploymentEnvProd}
    ...
    \item mount\_deploy\_part \texttt{[only with non-fsa and supported fs]}
  ...
  \item \texttt{SetDeploymentEnvUntrusted}
    ...
    \item mount\_deploy\_part \texttt{[only if non-fsa and supported fs]}
    \item format\_tmp\_part \texttt{[only if non-multipart]}
...
\item \texttt{BroadcastEnv}
  \begin{itemize}
    \item \texttt{BroadcastEnvChain}
      ...
      \item mount\_deploy\_part \texttt{[only with fsa and supported fs]}
