...
def _test_deploy(nodes, step1, step2, step3, test_name, key, env, kadeploy, max_simult)
  $stderr.puts("### Launch[#{test_name}/#{env}]")
...
def test_dummy(nodes, step1, step2, step3, test_name, kadeploy, env_list, max_simult)
  $stderr.puts("### Launch[#{test_name}/#{env_list.split(",")[0]}]")
  ok_file = Tempfile.new("blackboxtests-ok")
  ok = ok_file.path
  ko_file = Tempfile.new("blackboxtests-ko")
  ko = ko_file.path
  ...
  cmd = "#{kadeploy} #{node_list} -e \"#{env_list.split(",")[0]}\" --verbose-level 0 --force-steps \"SetDeploymentEnv|#{step1}&BroadcastEnv|#{step2}&BootNewEnv|#{step3}\" -o #{ok} -n #{ko} &1>2"
  start = Time.now.to_i
  system(cmd)
  time = Time.now.to_i - start
  if (count_lines(ko) > 0) then
