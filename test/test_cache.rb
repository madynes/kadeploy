...
class TestCache < Test::Unit::TestCase
  ...
  def test_default
    ...
    f1 = cache.cache(@files[0],'http://testbed.lan/FILE0','u1',hashs[0],Time.now,2,'pxe')
    f1dup = f1.dup
    f2 = cache.cache(@files[1],'/home/testuser/FILE1','u2',hashs[1],Time.now,1,'http')
    f3 = cache.cache(@files[2],'http://testbed.lan/FILE2','u1',hashs[2],Time.now,1,'pxe')
    f4 = cache.cache(@files[3],'http://testbed.lan/FILE0','u2',hashs[3],Time.now,2,'http')
    f4dup = f4.dup
    f5 = cache.cache(@files[4],'http://testbed.lan/FILE0','u3',hashs[4],Time.now,1,'env')
    f5dup = f5.dup
...
pp cache.files
  ...
  def test_lru
    ...
    f1 = cache.cache(@files[0],pathbase+'0','u1',hashs[0],Time.now,1,'pxe')
    sleep(1)
    f2 = cache.cache(@files[1],pathbase+'1','u2',hashs[1],Time.now,1,'http')
    sleep(1)
    f3 = cache.cache(@files[2],pathbase+'2','u2',hashs[1],Time.now,1,'http')
    sleep(1)
    f1.update_atime
    f4 = cache.cache(@files[3],pathbase+'3','u1',hashs[1],Time.now,1,'http')
...
pp cache.files
  ...
  def test_prio
    ...
    f1 = cache.cache(@files[0],pathbase+'0','u1',hashs[0],Time.now,2,'pxe')
    f2 = cache.cache(@files[1],pathbase+'1','u2',hashs[1],Time.now,1,'http')
    f3 = cache.cache(@files[2],pathbase+'2','u2',hashs[1],Time.now,1,'http')
    sleep(1)
    f2.update_atime
