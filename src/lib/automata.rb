...
=end
    ...
    def start()
      ...
      until (done?)
        ...
        query[:nsid] = curtask.nsid if !query[:nsid] and curtask
  ...
  class TaskedTaskManager < TaskManager
    ...
    def success_task(task,nodeset)
      ...
      if @config[task.name][:raisable]
        new_nsid = nil
        unless task.nodes.equal?(@nodes_ok)
          oldnsid = @nsid
          @nsid = Nodes::NodeSet.newid(context)
          new_nsid = Nodes::NodeSet.newid(context)

          split!(oldnsid,@nsid,task.nodes.diff(newns),new_nsid,@nodes_ok)
        end
    ...
    def fail_task(task,nodeset)
      ...
      if @config[task.name][:raisable]
        new_nsid = nil
        unless task.nodes.equal?(@nodes_ko)
          oldnsid = @nsid
          @nsid = Nodes::NodeSet.newid(context)
          new_nsid = Nodes::NodeSet.newid(context)

          split!(oldnsid,@nsid,task.nodes.diff(newns),new_nsid,@nodes_ko)
        end
