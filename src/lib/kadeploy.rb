...
module Kadeploy
  ...
  def deploy_bindings(info)
    bind(:deploy,info,'output','/outputs') do |httpd,path|
      httpd.bind([:GET,:HEAD],path,:method,
        :object=>self,:method=>:deploy_output,:args=>[info[:wid]])
    end

    bind(:deploy,info,'outputs','/outputs',info[:clusterlist]) do |httpd,path,cluster|
      httpd.bind([:GET,:HEAD],path,:method,
        :object=>self,:method=>:deploy_output,:args=>[info[:wid],cluster])
    end

    bind(:deploy,info,'state','/state') do |httpd,path|
      httpd.bind([:GET,:HEAD],path,:method,
        :object=>self,:method=>:deploy_state,:args=>[info[:wid]])
    end

    if info[:debugger]
      bind(:deploy,info,'debug','/debugs') do |httpd,path|
        httpd.bind([:GET,:HEAD],path,:method,
          :object=>self,:method=>:deploy_debug,:args=>[info[:wid]])
      end

      bind(:deploy,info,'debugs','/debugs',info[:nodelist]) do |httpd,path,node|
        httpd.bind([:GET,:HEAD],path,:method,
          :object=>self,:method=>:deploy_debug,:args=>[info[:wid],node])
      end
    end
  end
