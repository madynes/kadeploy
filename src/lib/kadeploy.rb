...
module Kadeploy
  ...
  def deploy_bindings(httpd,info)
    path = File.join(info[:wid],'output')
    httpd.bind([:GET,:HEAD],deploy_path(path)) do |request,method|
      deploy_output(info[:wid])
    end

    path = File.join(info[:wid],'state')
    httpd.bind([:GET,:HEAD],deploy_path(path)) do |request,method|
      deploy_state(info[:wid])
    end

    path = File.join(info[:wid],'debug')
    httpd.bind([:GET,:HEAD],deploy_path(path)) do |request,method|
  ...
  def deploy_output(wid,cluster=nil)
    get_workflow(:deploy,wid) do |info|
      if info[:workflows][cluster]
        info[:workflows][cluster].output.pop
      else
        output = ''
        output << info[:output].pop
        info[:workflows].each_value do |workflow|
          output << workflow.output.pop
        end
        output
      end
    end
