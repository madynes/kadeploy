...
module Kadeploy
  ...
  def deploy_get(wid=nil)
    workflow_get = Proc.new do |info|
      error = nil
      done = nil
      status = {}
    ...
    get_workflow(:deploy,wid) do |info|
      if info.is_a?(Array)
        ret = []
        info.each do |inf|
          tmp = workflow_get.call(inf)
          if tmp[1]
            deploy_delete(inf[:wid])
            raise tmp[1]
          end
          ret << tmp[0]
        end
        ...
        ret,err = workflow_get.call(info)
        if err
          deploy_delete(info[:wid])
          raise err
        end
        ret
      end
  ...
  def deploy_bindings(info)
    ...
    #  httpd.bind([:GET,:HEAD],path,:filter,
    #    :object=>self,:method=>:deploy_output,:args=>[1,3])
    #end
    ...
    #  httpd.bind([:GET,:HEAD],path,:filter,
    #    :object=>self,:method=>:deploy_state,:args=>[1])
    #end

    #if info[:debugger]
    #  bind(:deploy,info,'debug','/debugs') do |httpd,path|
    #    httpd.bind([:GET,:HEAD],path,:filter,
    #      :object=>self,:method=>:deploy_debug,:args=>[1,3])
    #  end
  ...
  def deploy_get_outputs(wid,cluster=nil)
