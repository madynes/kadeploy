...
=begin
    OPTIONS = {
      ...
      :dynamic => '--dynamic',
      :login => '--login',
      :self_propagate => '--self-propagate',
      :no_self_propagate => '--dont-self-propagate',
      :args_file => '--args-file',
      :gateway => '--gateway',
      :localname => '--localname',
      :send_files => '--send-files',
      :perl_interpreter => '--perl-interpreter',
      :taktuk_command => '--taktuk-command',
      :path_value => '--path-value',
    }
=end
    def to_cmd
      self.keys.inject([]) do |ret,opt|
        option = opt.to_s.gsub(/_/,'-').strip
        value = (self[opt].nil? ? '' : self[opt].to_s)
        ret << "--#{option}"
        ret << value
      end
    end
  ...
  class Commands < Array
    TOKENS=[
      'broadcast', 'downcast', 'exec', 'get', 'put',
      'input', 'data', 'file', 'pipe', 'close',
      'line', 'target', 'kill', 'message',
      'network', 'state', 'cancel', 'renumber', 'update',
      'option', 'synchronize', 'taktuk_perl', 'quit',
      'wait', 'reduce'
    ]
  ...
  class TakTuk
    ...
    def run!
      ...
      Execute.do(@binary,*@args) do |wait_thr,stdin,stdout,stderr|
        @stdout = stdout.read
        @stderr = stderr.read
