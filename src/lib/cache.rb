...
class CacheFile
  ...
  def save!(directory)
    if @filename != File.basename(@file)
      ...
      #FileUtils.mv(@file,filename) # Bugs for no reasons
      Execute["mv #{@file} #{filename}"].run!.wait
      @file = filename
      Execute["chmod #{MODE} #{@file}"].run!.wait
      #FileUtils.chmod(MODE,@file) # Bugs for no reasons
...
class Cache
  ...
  def write(file,path,version,user,priority,tag='') #md5?
    ret = nil
    unless get({
      #:file => file,
      :path => path,
  ...
  def cache(path,version,user,priority,tag='',md5=nil,mtime=nil)
    if ret = read({
      #:file => file,
      :path => path,
      :version => version,
      ...
      begin
        ...
        yield(file.path)
        file.close
        ret = write(file.path,path,version,user,priority,tag)
      rescue
        raise KadeployError.new(
...
=begin
  ...
  # !!! Be careful, use lock
  def cleanables!()
    @files.values.select{ |v| !v.used? }.sort_by{ |v| "#{v.priority}#{v.atime.to_i}".to_i }
  end
=end
