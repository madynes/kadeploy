...
class CacheFile
  ...
  attr_reader :file, :user, :priority, :path , :tag, :md5, :size, :atime, :mtime, :lock, :filename
...
class Cache
  ...
  def cache(file,path,user,priority,tag='',md5=nil,mtime=nil)
    if ret = read({
      ...
      save = false
      if priority != 0 \
        and ((mtime and mtime.call > ret.mtime) or !mtime) \
      ...
      then
        ...
        @lock.synchronize do
          ...
          delete!(ret)
          unless ret = add!(new)
            raise KadeployError.new(FetchFileError::FILE_TOO_BIG,nil,
              "Impossible to cache the file '#{file}', the file is too big"
            )
          end
        end
