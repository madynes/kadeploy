...
class CacheFile
  ...
  def initialize(file,path,prefix,user,priority,tag='')
    ...
    @file = file # The file in the filesystem
    @path = path # The path where the file is coming from (URL,...)
    @prefix = prefix
    @priority = priority
...
class Cache
  ...
  def write(file,path,user,priority,tag='')
    @lock.synchronize{ write!(file,path,user,priority,tag) }
  end
  ...
  def cache(file,path,user,priority,tag='',md5=nil,mtime=nil)
    if ret = read({
      :file => file,
      :path => path,
      :user => user,
      ...
      save = false
      if priority != 0 \
        and ((mtime and mtime > ret.mtime) or !mtime) \
        and (md5 and md5.call != ret.md5)
      then
        # The file has changed and is not single usage
        new = CacheFile.new(
          file,
          path,
          @prefix_base,
  ...
  def free!(amount=0)
    ...
    @files.values.each do |file|
      delete!(file) if file.priority == 0 and !file.lock.locked?
    end
