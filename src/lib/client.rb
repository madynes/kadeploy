...
class Client
  ...
  def get(path)
    HTTPClient::get(@server,@port,path,@secure,self.class.service_name)
  end

  def post(path,data,content_type='application/json')
    HTTPClient::post(@server,@port,path,data,@secure,content_type,self.class.service_name)
  end

  def delete(path)
    HTTPClient::delete(@server,@port,path,@secure,self.class.service_name)
  end
  ...
  def self.httpd_bind_files(files)
    files.each do |file|
      ...
      obj = File.new(file)
      $files << obj
      $httpd.bind([:HEAD,:GET],"/#{Base64.urlsafe_encode64(file)}",obj)
    end
  end
