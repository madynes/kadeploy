...
class HTTPClient
  ...
  def self.request(server,port,path,service='Client')
    res = nil
  ...
  def self.get(server,port,path,service='Client')
    res = request(server,port,path,service) { |client| client.get(path) }
    if block_given?
      yield(res)
      res = nil
    end
  ...
  def self.post(server,port,path,data,content_type='application/json',service='Client')
    res = request(server,port,path,service) do |client|
  ...
  def self.delete(server,port,path,service='Client')
    res = request(server,port,path,service) { |client| client.delete(path) }
    if block_given?
      yield(res)
      res = nil
    end
...
class Client
  ...
  def get(path)
    HTTPClient::get(@server,@port,path,self.class.service_name)
  end

  def post(path,data,content_type='application/json')
    HTTPClient::post(@server,@port,path,data,content_type,self.class.service_name)
  end

  def delete(path)
    HTTPClient::delete(@server,@port,path,self.class.service_name)
  end
