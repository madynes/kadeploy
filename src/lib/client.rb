...
class Client
  ...
  def api_path(path='',api_dir=API_DIR)
    if @resources
      ...
      ret = File.join("#{(@secure ? 'https' : 'http')}://","#{@server}:#{@port}",api_dir)
      ret = File.join(ret,path) if path
      ret
    end
  end
  ...
  def get(uri,params=nil)
    ...
    path = HTTPClient.path_params(path,params) if params
  ...
  def self.get_nodelist(server,port,secure=true)
    ...
    begin
      Timeout.timeout(8) do
        path = HTTPClient.path_params('/nodes',{:user => USER})
