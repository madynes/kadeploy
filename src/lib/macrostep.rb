...
class Macrostep < Automata::TaskedTaskManager
  ...
  def break!(task,nodeset)
    ...
    debug(1,"Step #{self.class.name} breakpointed for nodes #{nodeset.to_s_fold}",task.nsid)
  end

  def success!(task,nodeset)
    #debug(4,"<<< Raising OK nodes #{nodeset.to_s_fold} from #{self.class.name}") if @config[task.name][:raisable]
    debug(1,
      "End of step #{self.class.name} for nodes #{nodeset.to_s_fold} "\
  ...
  def fail!(task,nodeset)
    ...
    debug(1,
      "Step #{self.class.name} failed for nodes #{nodeset.to_s_fold} "\
      "after #{Time.now.to_i - @start_time}s",
      task.nsid
    )
  end
  ...
  def split!(nsid0,nsid1,ns1,nsid2,ns2)
    debug(1,"Nodeset(#{nsid0}) split into :")
    debug(1,"  Nodeset(#{nsid1}): #{ns1.to_s_fold}")
    debug(1,"  Nodeset(#{nsid2}): #{ns2.to_s_fold}")
  end
