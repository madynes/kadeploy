...
class Workflow < Automata::TaskManager
  ...
  def start!
    ...
    debug(0, "Launching a deployment on cluster #{context[:cluster].name} ...")
    context[:dblock].lock
  ...
  def break!(task,nodeset)
    ...
    debug(1,"Breakpoint reached for nodes #{nodeset.to_s_fold}",task.nsid)
  end
  ...
  def success!(task,nodeset)
    ...
    debug(1,
      "End of deployment for nodes #{nodeset.to_s_fold} "\
  ...
  def fail!(task,nodeset)
    ...
    debug(1,
      "Deployment failed for nodes #{nodeset.to_s_fold} "\
      "after #{Time.now.to_i - @start_time}s",
      task.nsid
    )
  end
  ...
  def done!()
    ...
    unless @nodes_ok.empty? and @nodes_brk.empty?
      debug(0, "Nodes correctly deployed on cluster #{context[:cluster].name}")
      debug(0, @nodes_brk.to_s(false, false, "\n")) unless @nodes_brk.empty?
      debug(0, @nodes_ok.to_s(false, false, "\n")) unless @nodes_ok.empty?
    end

    unless @nodes_ko.empty?
      debug(0, "Nodes not correctly deployed on cluster #{context[:cluster].name}")
      debug(0, @nodes_ok.to_s(false, true, "\n"))
    end
