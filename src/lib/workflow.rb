...
module Workflow
  class Workflow < Automata::TaskManager
    ...
    def start!
      ...
      debug(0, "Launching a #{self.class.operation()} on #{@nodes.to_s_fold}")
      ...
      unless context[:execution].force
        ...
        unless to_discard.empty?
          debug(0,
            ...
            "#{self.class.operation()}, let's discard them"
          )
    ...
    def success!(task,nodeset)
      ...
      log('success', true, nodeset)
      debug(1,
        "End of #{self.class.operation()} for #{nodeset.to_s_fold} "\
    ...
    def done!()
      debug(0,
        "End of #{self.class.operation()} on cluster #{context[:cluster].name} "\
        "after #{Time.now.to_i - @start_time}s"
      )
    ...
    def kill!()
      ...
      debug(0,"#{self.class.operation()} aborted by user")
    end
