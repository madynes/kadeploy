...
=end
  ...
  def ms_decompress_environment(scattering_kind)
    ...
    complvl = context[:execution].environment.image[:compression]
    fsmap = context[:execution].environment.image[:options] #TODO: ...
    return parallel_exec(
      "fsarchiver -j #{nbcpu} -z #{complvl} restfs #{fsarchive()} #{fsmap}",
      { :scattering => op[:scattering] }
    )
  end
