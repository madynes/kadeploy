...
class ProcedureHandler < WEBrick::HTTPServlet::AbstractServlet
...
class MethodHandler < WEBrick::HTTPServlet::AbstractServlet
  ...
  def initialize(methods,obj,meth)
    @obj = obj
    @method = meth.to_sym
    methods.each do |m|
      alias_method :"do_#{m.to_s.upcase}", :do_METHOD
    end
  end
  ...
  def do_METHOD(request, response)
    ...
    response['Cache-Control'] = 'no-store, no-cache'
    response['Pragma'] = 'no-cache'
    args = request.request_uri.path.split('/')
    @obj.send(@method,*args)
  end
end

class GetMethodHandler < MethodHandler
  alias do_GET do_METHOD
end

class PostMethodHandler < MethodHandler
  alias do_POST do_METHOD
end
